...
<template>
  <header
    class="flex h-[72px] flex-none items-center justify-between gap-3 border-b border-hairline
           bg-surface/90 px-5 backdrop-blur-sm sm:px-6"
  >
    <div class="flex min-w-0 items-center gap-3">
      ...
      <div class="min-w-0">
        ...
        <p v-if="subtitle" class="muted truncate text-[12.5px]">{{ subtitle }}</p>
      </div>
    </div>
    <div v-if="$slots.actions" class="flex flex-none items-center gap-2.5">
      <slot name="actions" />
    </div>
  </header>
