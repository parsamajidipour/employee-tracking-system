  ...
  <AppShell :title="item?.title ?? t('cases.detail.workspace')" :subtitle="item?.reference_no" back-to="/cases" full-bleed>
    ...
    <div v-else-if="item" class="flex h-full min-h-0 flex-col gap-3 overflow-y-auto p-3 sm:gap-4 lg:overflow-hidden lg:p-5">
      ...
      <div class="grid flex-none grid-cols-1 gap-4 lg:min-h-0 lg:flex-1 xl:grid-cols-[minmax(0,1.05fr)_minmax(340px,.9fr)_minmax(360px,.95fr)]">
        ...
        <Card class="min-h-[560px] xl:min-h-0" icon="users" :title="canAssign ? (item.status === 'rejected' ? t('cases.detail.reassign') : t('cases.detail.assign')) : t('cases.detail.assignment')" flush>
          ...
          <div v-if="canAssign" class="flex h-full min-h-0 flex-col">
            ...
            <div class="min-h-0 flex-1 overflow-y-auto">
              ...
              <label v-for="choice in surveyorChoices" v-else :key="choice.employee.id" class="surveyor-option group grid cursor-pointer grid-cols-[minmax(0,1fr)_42px_76px] items-center gap-3 border-b border-s-4 border-hairline border-s-transparent px-3 py-3 transition-colors last:border-b-0" :class="selectedSurveyorIds.includes(choice.employee.id) ? '!border-s-primary bg-primary-soft' : ''">
